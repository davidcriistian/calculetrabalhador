...
const PAGE_URL = 'http://127.0.0.1:4173/blog/como-organizar-as-financas-sendo-pj/';
const DEBUG_URL = 'http://127.0.0.1:9222';
const OUTPUT_DIR = path.join(ROOT, 'data', 'operations', 'reports', 'end-to-end', 'create-article-como-organizar-financas-pj', 'visual');
...
async function run() {
  ...
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'ct-edge-article-financas-pj-'));
  ...
  try {
    ...
    const states = [['entry', 'window.scrollTo(0,0)'], ['summary-toc', 'document.querySelector("#sumario-title").scrollIntoView({block:"start"})'], ['cashflow-table', 'document.querySelector("#fluxo-caixa").scrollIntoView({block:"start"})'], ['plan-faq', 'document.querySelector("#plano-pratico").scrollIntoView({block:"start"})'], ['sources-footer', 'document.querySelector("#fontes-title").scrollIntoView({block:"start"})']];
    for (const [name, width, height] of viewports) {
      ...
      await wait(2500);
      const ready = await evaluate(send, '({ready:document.readyState,h1:document.querySelectorAll("h1").length,bodyWidth:document.body.scrollWidth,viewport:innerWidth,header:!!document.querySelector("#global-header header"),footer:!!document.querySelector("#global-footer footer")})');
      if (ready.ready !== 'complete' || ready.h1 !== 1 || !ready.header || !ready.footer || ready.bodyWidth > ready.viewport + 1) throw new Error(`${name} readiness/overflow failed: ${JSON.stringify(ready)}`);
      for (const [state, expression] of states) {
        await evaluate(send, `${expression};true`); await wait(250); await screenshot(send, `${name}-${state}`);
      }
    }
    console.log('Como organizar as finanças sendo PJ visual matrix: 4 viewports × 5 sections PASS');
  } finally {
    ...
    if (path.dirname(path.resolve(profile)).toLowerCase() === tempRoot.toLowerCase() && path.basename(profile).startsWith('ct-edge-article-financas-pj-')) {
